...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#dg").datagrid({
			idField:"id",
			url:"RoleServlet?opType=queryByPage",
	...
	function add(){
        ...
        $("#ff").form("reset");
		//新增记录的请求地址
		url = "RoleServlet?opType=add";
		//打开弹出框
		openFormDialog();
		//设置弹出框标题
		$("#dd").dialog("setTitle","新增角色信息");
	}
	//修改记录
	function update(){
		var array = $("#dg").datagrid("getSelections");
        ...
        $("#ff").form("reset");
      	//修改记录的请求地址
		url = "RoleServlet?opType=update&id="+array[0].id;
		//表单填充内容
        $("#ff").form("load",array[0]);
      	//打开弹出框
		openFormDialog();
		//设置弹出框标题
		$("#dd").dialog("setTitle","修改角色信息");
	}
	//保存或者更新数据
	function save(){
		$("#ff").form("submit",{
